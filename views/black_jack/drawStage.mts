...
import {Card} from "./sprites/Card.mjs"

export class DrawStage{
    scene: string = ""
    stage

    CARDDESIGN = {x: 30, y: 40, line: 0x000000, fill: 0xffffff}
    MARK: string[] = ["" ,"♣", "♠", "♦", "♥"]
    ...
    changeScene(new_scene: string, old_scene: string): void {
    ...
    drawCard(card: Card, x: number, y: number): void {
        ...
        let text: PIXI.Text = new PIXI.Text(`${this.MARK[card.mark]} ${card.num}`,{fontFamily : 'Noto Sans JP', fontSize: 12, fill : 0xf000000, align : 'center'})
        text.x = x
        text.y = y
